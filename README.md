...
> **Note on this copy:** this particular build has been re-namespaced to use its own internal module name (`summaryception_eteitaxiv_fork`) instead of the shared `summaryception` key. That means it will **not** carry over settings/memory from — or get clobbered by resets of — the original extension or other forks. If you want the shared-settings behavior described above, use an unmodified copy instead.
